...
We can see from the plot, we do not have any high negative correlation, but we do have many high positive correlation. Such as the correlation coefficient between Folate_mcg and Thiamin_mg is 1, which is perfect positive correlation, and others like Protein_g and Zinc_mg have a correlation coefficient of 0.9 which is considered high correlation.
...
pc1 <- ggplot(pca_loadings, aes(x = reorder(variable, abs(PC1)),
                                y = PC1)) +
...
pc2 <- ggplot(pca_loadings, aes(x = reorder(variable, abs(PC2)),
                                y = PC2)) +
...
pc3 <- ggplot(pca_loadings, aes(x = reorder(variable, abs(PC3)),
                                y = PC3)) +
...
pc1_new <-
  ggplot(pca_loadings_new, aes(x = reorder(variable, abs(PC1)),
                               y = PC1)) +
  geom_bar(stat = 'identity', fill = "#FF6666") +
  theme(axis.text.x = element_text(
    angle = 50,
...
pc2_new <-
  ggplot(pca_loadings_new, aes(x = reorder(variable, abs(PC2)),
                               y = PC2)) +
  geom_bar(stat = 'identity', fill = "darkgreen") +
  theme(axis.text.x = element_text(
    angle = 50,
...
pc3_new <-
  ggplot(pca_loadings_new, aes(x = reorder(variable, abs(PC3)),
                               y = PC3)) +
  geom_bar(stat = 'identity', fill = "blue") +
  theme(axis.text.x = element_text(
    angle = 50,
